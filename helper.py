...
def load_dataset(dataset_name = 'svhn', split='teach', make_normal=False, randomize=False):
    """
    ...
    :param randomize: boolean specifying whether or not to randomize the partitions, defaults to False
    ...
    if dataset_name == 'svhn':
        if split == 'teach':
            ...
            if randomize:
                train_dataset, valid_dataset = torch.utils.data.random_split(train_dataset, [train_length, valid_length], generator = torch.Generator().manual_seed(0))
            else:
