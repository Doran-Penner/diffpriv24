...
def student_train(training_data,valid_data,lr_start=1e-2,epochs=70,batch_size=256,net=BBB3Conv3FC):
    """
    based on:
...
def active_learning(network=BBBAlexNet,acquisition_iterations=100,initial_size=100,acquisition_method=BatchBALD,num_acquisitions=20,print_summary=True,epochs=100):
    ...
    test_dict["epsilon"].append(gnmax_epsilon(data_dep_eps_costs,alpha=agg.alpha,sigma=agg.scale,delta=1e-6))
    print(gnmax_epsilon(data_dep_eps_costs,alpha=agg.alpha,sigma=agg.scale,delta=1e-6))
    ...
    for round in range(acquisition_iterations):
        ...
        test_dict["valid_loss"].append(valid_loss)
        test_dict["test_acc"].append(calculate_test_accuracy(model,dat_obj.student_test))
        test_dict["epsilon"].append(gnmax_epsilon(data_dep_eps_costs,alpha=agg.alpha,sigma=agg.scale,delta=1e-6)) # TODO possibly change this for abstraction later!
