...
class BBBAlexNet(ModuleWrapper):
    ...
    def __init__(self, dat_obj, priors = [None]*6, layer_type='lrt', activation_type='softplus'):
...
#        else:            raise ValueError("Undefined layer_type")
        ...
        self.conv1 = BBBConv2d(shape[2], 64, 11, stride=4, padding=5, bias=True, priors=self.priors[0])
        self.act1 = self.act()
        self.pool1 = nn.MaxPool2d(kernel_size=2, stride=2)

        self.conv2 = BBBConv2d(64, 192, 5, padding=2, bias=True, priors=self.priors[1])
        self.act2 = self.act()
        self.pool2 = nn.MaxPool2d(kernel_size=2, stride=2)

        self.conv3 = BBBConv2d(192, 384, 3, padding=1, bias=True, priors=self.priors[2])
        self.act3 = self.act()

        self.conv4 = BBBConv2d(384, 256, 3, padding=1, bias=True, priors=self.priors[3])
        self.act4 = self.act()

        self.conv5 = BBBConv2d(256, 128, 3, padding=1, bias=True, priors=self.priors[4])
        self.act5 = self.act()
        self.pool3 = nn.MaxPool2d(kernel_size=2, stride=2)

        self.flatten = FlattenLayer(1 * 1 * 128)
        self.classifier = BBBLinear(1 * 1 * 128, self.num_classes, bias=True, priors=self.priors[5])
